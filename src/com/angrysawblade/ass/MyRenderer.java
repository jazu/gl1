...
public class MyRenderer extends GLSurfaceView implements Renderer {
	...
	// IF TIME, add menu/game music and sound effect
	...
	float scrw = 0.0f;
	float scrh = 0.0f;
	float virtualscrw = 0.0f;
	float virtualscrh = 0.0f;
	...
	MediaPlayer mp = MediaPlayer.create(getContext(), R.raw.menumusicorig);
	...
	public boolean initMusic() {
		if(mp.isPlaying() == false && gameState == STATE_MENU) {
			mp.start();
		return true;	
		}
		return false;
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		ball.setTextureCoords(StextureCoords); 		// load textures
		...
		for ( int i = 0; i < 10; i++ ) { //load score textures from array
			numbers[i] = new Square(100, 100);
			numbers[i].setTextureCoords(StextureCoords);
			int rid = getResources().getIdentifier("score"+String.valueOf(i), "drawable", "com.angrysawblade.ass");
	...
	public void onDrawFrame(GL10 gl) {
		...
		gl.glLoadIdentity();
		gl.glEnable(GL10.GL_TEXTURE_2D);  //needed only the for emulator to draw the game	
		...
		if(gameState == STATE_MENU) {
			...
			gl.glPushMatrix();
			gl.glTranslatef(virtualscrw/2, virtualscrh/2, 0.0f);
	  		bg.draw(gl);
			gl.glPopMatrix();
			...
			gl.glPushMatrix();
			gl.glTranslatef(virtualscrw/1.2f, virtualscrh/2, 0.0f);
			gl.glRotatef(angle, 0.0f, 0.0f, 0.0f);
			menuabout.draw(gl);
			gl.glPopMatrix();
			
			gl.glPushMatrix();
			gl.glTranslatef(virtualscrw/6, virtualscrh/2, 0.0f); 
			gl.glRotatef(angle, 0.0f, 0.0f, 0.0f);
			menuhelp.draw(gl);
			gl.glPopMatrix();
		...
		if(gameState == STATE_ABOUT) {
			...
			gl.glPushMatrix();
			gl.glTranslatef(virtualscrw/17.066f, virtualscrh/2, 0.0f);
			aboutback.draw(gl);
			gl.glPopMatrix();
		}
		...
		if(gameState == STATE_STARTED) {
				...
				if(bally >= virtualscrh-(squarew/2) || bally <= (squarew/2)) { //accelerate ball after it touches pads 
					ballyv = -ballyv*1.02f;
				}
				if((ballx <= (padw/2)+(squarew/2) && bally <= lpady+(padh/2)+(squareh/2) && bally >= lpady-(padh/2)-(squareh/2)) || //check pad-ball collision  
				   (ballx >= virtualscrw-(padw/2)-(squarew/2) && bally <= rpady+(padh/2)+(squareh/2) && bally >= rpady-(padh/2)-(squareh/2))) { 
					ballx = (padw/2)+(squarew/2);
					ballxv = -ballxv*1.1f; 
					
					if(ballxv > 0 )	{		//check that ball is not inside the pads
						ballx = (padw/2)+(squarew/2);
						}
						else if(ballxv < 0){ 
							ballxv = +ballxv*1.1f; 
							ballx = virtualscrw-(padw/2)-(squarew/2);
							}
					}
				...
				gl.glPushMatrix();
				gl.glTranslatef(virtualscrw-virtualscrw, padstarty, 0.0f);
				...
				gl.glTranslatef(virtualscrw, rpady, 0.0f);
				rpad.draw(gl);
				gl.glPopMatrix();
		...
		if(gameState == STATE_GAMEOVER) {
				...
				gl.glPushMatrix();
				gl.glTranslatef(virtualscrw/1.2f, virtualscrh/1.2f, 0.0f);
				replay.draw(gl);
				gl.glPopMatrix();
				
				gl.glPushMatrix();
				gl.glTranslatef(virtualscrw/1.15f, virtualscrh/5.2f, 0.0f);
				scorebg.draw(gl);
				gl.glPopMatrix();
				
				// rscore
				gl.glPushMatrix();
				gl.glTranslatef(virtualscrw/1.1f, virtualscrh/5.2f, 0.0f);
				numbers[rscore].draw(gl);
				gl.glPopMatrix();
				
				// lscore
				gl.glPushMatrix();
				gl.glTranslatef(virtualscrw/1.2f, virtualscrh/5.2f, 0.0f);
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		ballstartx = virtualscrw/2;
		ballstarty = virtualscrh/2;
		ballx = ballstartx;
		bally = ballstarty;
	} 
	@Override
	public boolean onTouchEvent(MotionEvent event) {
		...
		if(gameState == STATE_STARTED) {    								//multi touch
				...
				case MotionEvent.ACTION_MOVE: {
						for (int i = 0; i < pointerCount; i++) {
							if(event.getX(i) < lside) {
								lpady = scrh-event.getY(i);
							} else{
								rpady = scrh-event.getY(i);
							}
						}
						return true;
		        }
